...
class ExamController extends Controller
{
    ...
    public function removeExam(Request $request)
    {
        ...
        if($result){
            ...
            'This quiz isplayed by user so it cannot be remove!');
        } else {
            $quiz->users()->detach($userId);
            return redirect()->back()->with('message', 
            'Exam is now not assigned to that user!');
        }
    }
    ...
    public function getQuizQuestions(Request $request, $quizId)
    {
        ...
        //check if user has benn assigned a particular quiz
    ...
    public function userQuizResult($userId, $quizId)
    {
        ...
        $attemptQuestion = Result::where('quiz_id', $quizId)->where('user_id')->count();
        ...
        $userCorrectedAnswer = Answer::where('id', $ans)->where('is_correct', 1)->count();
        ...
        return view('backend.result.result', compact('results', 'totalQuestions', 'attemptQuestion',
